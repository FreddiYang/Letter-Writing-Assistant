...
function generateFallbackResponse(messages: ChatMessage[]): { reply: string; letterDraft?: string | null } {
...
Once you confirm your preferences, I will draft your formal letter.`,
  ...
  // Rule 5: User answered clarification or requested draft
  // Check tone choice
  let tone = 'firm-but-respectful';
  if (latestLower.includes('diplomatic')) tone = 'diplomatic';
  if (latestLower.includes('urgent')) tone = 'urgent';

  const isAIOrAcademic = messages.some((m) =>
  ...
  const isBillingOrService = messages.some((m) =>
...
async function startServer() {
  ...
  app.post("/api/chat", async (req: Request, res: Response) => {
    try {
      ...
      // Check if letter draft is enclosed in <draft_letter>
      ...
      // If error occurs, gracefully fallback so user experience is uninterrupted
      const { messages } = req.body as { messages: ChatMessage[] };
